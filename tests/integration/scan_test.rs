...
fn scan_finds_axios_vuln() {
    ...
    cmd.args(["scan", fixtures_dir().join("npm-axios-vuln").to_str().unwrap()])
        .assert()
        .success()
...
fn scan_finds_no_issues_in_fixed_repo() {
    let mut cmd = Command::cargo_bin("dep-doctor").unwrap();
    cmd.args(["scan", fixtures_dir().join("npm-axios-fixed").to_str().unwrap()])
        .assert()
        .success()
...
fn scan_python_requests_vuln() {
    let mut cmd = Command::cargo_bin("dep-doctor").unwrap();
    cmd.args(["scan", fixtures_dir().join("python-requests").to_str().unwrap()])
        .assert()
        .success()
